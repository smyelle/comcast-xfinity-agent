...
"""
  ...
  # Deterministically set the technician type and, for the appointment path only,
  # the dispatch codes. A "Network Tech" recommendation is a LIVE-HUMAN handoff
  # and must NOT carry any appointment/truck-roll ticket data.
  matched_activity_type = ""
  matched_activity_code = ""
  matched_job_type = ""
  matched_problem_code = ""
  matched_intents = ""
  matched_technician_type = ""
  ...
  if isinstance(inner_data, dict):
    ...
    if isinstance(rec, dict):
      ...
      if tech == "network tech":
        # Human handoff — leave all appointment dispatch fields empty.
        matched_technician_type = "network_tech"
        is_network_tech = True
        ...
        matched_technician_type = "install_repair_tech"

  # Apply appointment defaults only when this is NOT a network-tech (human)
  # handoff — a human transfer carries no ticket data.
  ...
  try:
    ...
    if matched_technician_type:
      context.state["technician_type"] = matched_technician_type
    # For a Network Tech (live-human) handoff we don't set any appointment ticket
    # fields; only the appointment path writes activityCode/jobType/etc.
    if not is_network_tech:
