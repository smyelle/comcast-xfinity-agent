  ...
  try:
    if final_response and isinstance(final_response, list):
      ...
      if sanitized or duplicate_dropped:
        text_parts = [Part(text=t) for t in deduped_texts if t and t.strip()]

        # Safety net: if no visible text parts remain, inject contextual message
        if not text_parts:
          replacement = _get_contextual_replacement(callback_context)
          text_parts = [Part(text=replacement)]
          print(f"[after_agent_callback] All text parts were empty after sanitization. Injected contextual message: '{replacement}'")
